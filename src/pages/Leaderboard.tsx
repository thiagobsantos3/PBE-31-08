...
export function Leaderboard() {
  ...
  const teamStats = React.useMemo(() => {
    ...
    const totalQuizzes = leaderboardData.reduce((sum, member) => sum + member.totalQuizzesCompleted, 0);
    const totalQuestions = leaderboardData.reduce((sum, member) => sum + member.totalQuestionsAnswered, 0);
    const totalTime = leaderboardData.reduce((sum, member) => sum + member.totalTimeSpentMinutes, 0);
    const totalPointsEarned = leaderboardData.reduce((sum, member) => sum + member.totalPointsEarned, 0);
    const totalPointsPossible = leaderboardData.reduce((sum, member) => sum + member.totalPossiblePoints, 0);
